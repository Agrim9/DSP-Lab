...
Int16 den_coeff[ORDER_IIR]={100,-160,64};
Int16 num_coeff[ORDER_IIR]={10000,-12728,8100};
...
{	
	...
	for(j=0;j<3;j++)
	{
	    ...
	    acc+=num_coeff[j]*x_IIR[(abs)%3];
	    if(j!=0)
	    {
	        acc-=den_coeff[j]*y_IIR[(abs)%3];
	    }
	}
	acc=(acc/den_coeff[0])/(32768.0);
