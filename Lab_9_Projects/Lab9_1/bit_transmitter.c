...
//bit 0 is sent as a 500Hz square wave
Int16 data_zero[96] = {
...
//bit 1 is sent as a 250Hz square wave
Int16 data_one[192] = {
 ...
 *  In this function we will be sending 6 cycles for bit 1                  *
 *                                                                          *
 * ------------------------------------------------------------------------ */
Int16 send_one()
{
 ...
 *  In this function we will be sending 6 cycles for bit 0                  *
 *                                                                          *
 * ------------------------------------------------------------------------ */
Int16 send_zero()
{
...
{
    Int16 i=0;
    Int16 data_to_send[11] = {1,1,0,1,0,0,0,0,0,0,0};//11010 is the preamble, 000000 is the data
    Int16 data_count = 0;
    Int16 values_sent = 0;
    Int16 xmit_seq = 0; 
    /* Initialize BSL */      
    ...
    //initially make the line free so that the receiver can detect the sudden transitions in the line when data is being sent	
    for(i = 0; i<10; i++)
    {
	    	LEFT_DATA = xmit_seq; 
	    	RIGHT_DATA = xmit_seq;  
    ...
    while(TRUE)
    {
	    ...
	    if( data_to_send[data_count] == 1 )
	    {
	    	values_sent = send_one();
		      //after sending a bit make the line free for receiver to detect next bit
		      //Here the value 1000 is selected arbitrarily. You can try to vary this or remove the loop and test if you can get
